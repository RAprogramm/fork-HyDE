...
if [ "$enableWallDcol" -eq 0 ] && [[ $reload_flag -eq 1 ]]; then
    ...
    parallel fn_wallbash {} "${wallbashDirs[@]}" ::: "${deployList[@]}" || render_failures=$?
elif [ "$enableWallDcol" -gt 0 ]; then
    print_log -sec "wallbash" -stat "apply $dcol_mode colors" "Wallbash theme"
    find -H "${wallbashDirs[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | parallel fn_wallbash {} "${wallbashDirs[@]}" || render_failures=$?
fi
find -H "${wallbashDirs[@]}" -type f -path "*/always*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | parallel fn_wallbash {} "${wallbashDirs[@]}" || render_failures=$?
